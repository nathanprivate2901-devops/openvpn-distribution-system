-- ============================================================================
-- OpenVPN Distribution System - Database Schema
-- ============================================================================
-- This script creates the complete database schema for the OpenVPN
-- Distribution System including all tables, indexes, foreign keys,
-- and initial data.
--
...
DROP DATABASE IF EXISTS openvpn_system;
...
CREATE INDEX idx_qos_policies_name ON qos_policies(name);
CREATE INDEX idx_qos_policies_priority ON qos_policies(priority);
...
CREATE INDEX idx_config_files_user_id ON config_files(user_id);
CREATE INDEX idx_config_files_qos_policy_id ON config_files(qos_policy_id);
CREATE INDEX idx_config_files_filename ON config_files(filename);
CREATE INDEX idx_config_files_revoked_at ON config_files(revoked_at);
CREATE INDEX idx_config_files_downloaded_at ON config_files(downloaded_at);
...
CREATE INDEX idx_user_qos_user_id ON user_qos(user_id);
CREATE INDEX idx_user_qos_qos_policy_id ON user_qos(qos_policy_id);
CREATE INDEX idx_user_qos_assigned_at ON user_qos(assigned_at);
...
VALUES (
  'admin',
  'admin@example.com',
  '$2b$10$rZ7pqYhE6eYnDXNhxXvkVOYFZmF5VhxX7.LCqvpQY4h0VqGvfPfTy', -- Hash for 'admin123'
...
-- ============================================================================
-- Initial Data: Sample QoS Policies
-- ============================================================================
-- Creates default QoS policies for different user tiers
-- ============================================================================

INSERT INTO qos_policies (name, description, bandwidth_limit, priority, created_at, updated_at)
VALUES
  ('Basic', 'Basic tier with standard bandwidth allocation', 5120, 'low', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('Standard', 'Standard tier with enhanced bandwidth', 10240, 'medium', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('Premium', 'Premium tier with maximum bandwidth and priority', 20480, 'high', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
  ('Enterprise', 'Enterprise tier with unlimited bandwidth', 51200, 'high', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
SELECT COUNT(*) AS UserTables FROM information_schema.tables
  WHERE table_schema = 'openvpn_system' AND table_type = 'BASE TABLE';
SELECT COUNT(*) AS Views FROM information_schema.tables
  WHERE table_schema = 'openvpn_system' AND table_type = 'VIEW';
SELECT COUNT(*) AS StoredProcedures FROM information_schema.routines
  WHERE routine_schema = 'openvpn_system' AND routine_type = 'PROCEDURE';

-- Display initial admin credentials (for reference)
SELECT
  '=====================================' AS '',
  'DEFAULT ADMIN CREDENTIALS' AS '',
  '=====================================' AS ' ',
  'Email: admin@example.com' AS '  ',
  'Password: admin123' AS '   ',
  '=====================================' AS '    ',
  'CHANGE THIS PASSWORD IMMEDIATELY!' AS '     ',
  '=====================================' AS '      ';

-- Display sample QoS policies
SELECT
  '=====================================' AS '',
  'SAMPLE QOS POLICIES CREATED' AS ' '
UNION ALL
SELECT '=====================================' AS '', '' AS ' ';

SELECT name, description, bandwidth_limit, priority FROM qos_policies;
