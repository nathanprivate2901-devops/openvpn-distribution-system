...
**Keywords:** Virtual Private Network, OpenVPN, Web Application, Automation, Security, Full-Stack Development, Node.js, Next.js, Docker, Enterprise System, Network Management
...
4. **Limited Visibility**: Traditional deployments provide minimal insight into device usage patterns, connection history, or user activity, hindering capacity planning and security monitoring.

5. **Scalability Constraints**: Manual processes that function adequately for small teams become prohibitively time-intensive for organizations with hundreds of users, multiple locations, or high user turnover.
...
1. **Authentication and Authorization:** Implementing secure user authentication with email verification, role-based access control, and integration with existing VPN infrastructure
2. **Automated Synchronization:** Designing reliable mechanisms to synchronize user accounts between MySQL database and OpenVPN Access Server without manual intervention
3. **Configuration Generation:** Dynamically generating valid OpenVPN configuration files with user-specific parameters and routing rules
...
This thesis focuses on OpenVPN Access Server due to its enterprise-ready features, extensive documentation, and API-based management capabilities. The system communicates with Access Server through a secure proxy protocol, eliminating the need for direct Docker socket access while maintaining full programmatic control over user management and configuration operations.
...
1. **Pritunl:** Web-based OpenVPN management interface with MongoDB backend. Provides graphical management but lacks automated MySQL synchronization features central to this thesis (Pritunl, 2024).
...
This thesis contributes a production-ready, open-source solution providing comprehensive automation, MySQL integration, and flexible device management without commercial licensing costs or vendor dependencies.
...
│                    INTEGRATION LAYER                         │
│  ┌────────────────────────────────────────────────────────┐ │
│  │        VPN Profile Proxy Server (Node.js)              │ │
│  │  - Authentication & Authorization                      │ │
│  │  - Request Validation & Sanitization                   │ │
│  │  - Rate Limiting & Security                            │ │
│  │  - Command Translation (HTTP → sacli)                  │ │
│  └────────────────────────────────────────────────────────┘ │
│                              │                               │
│                              │ sacli commands                │
│                              ▼                               │
...
**6. Proxy Pattern:**
- VPN Profile Proxy acts as intermediary between backend and OpenVPN Access Server
- Translates HTTP requests to sacli command-line operations
- Provides security layer and abstraction from underlying implementation
- Enables secure communication without Docker socket exposure

### 3.2.1 VPN Profile Proxy Protocol Architecture

The system implements a **secure proxy protocol** through the VPN Profile Proxy service to eliminate Docker socket security vulnerabilities while maintaining full OpenVPN Access Server control:
...
Backend Application (Port 3000)
        ...
        ▼
VPN Profile Proxy (Port 8088)
        │
        ├── Authentication Layer
        │   └── Verify API key/token
        │
        ├── Validation Layer
        │   └── Sanitize inputs, validate parameters
        │
        ├── Rate Limiting Layer
        │   └── Prevent abuse, throttle requests
        │
        ├── Command Translation Layer
        │   └── Convert HTTP → sacli commands
        │
        └── Execution Layer
            └── Execute sacli via local system access
                    │
                    ▼
            OpenVPN Access Server
```
...
- **Rate Limiting**: Per-endpoint throttling preventing abuse
- **Audit Logging**: Complete request/response logging for security monitoring
- **Isolated Execution**: Proxy runs in separate container with minimal privileges
- **No Socket Exposure**: Backend never accesses Docker socket directly

**API Endpoints:**
```
POST   /api/users/create          # Create OpenVPN user
PUT    /api/users/:username        # Update user properties
...
- **Enums**: Constrain values for roles, device types
...
- **HTTP Client**: axios 1.7+ (for proxy communication)
...
- Create/update/delete users via VPN Profile Proxy API
- Temporary password generation
- Non-blocking operations with axios
- Comprehensive error handling
- Sync history tracking
- Retry logic with exponential backoff
```

**3. VPN Profile Proxy Communication:**
```javascript
// proxyClient.js - Features:
- HTTP/HTTPS requests to VPN Profile Proxy
...
- RESTful API endpoints for OpenVPN operations
- API key authentication middleware
- Input validation and sanitization
- Rate limiting per endpoint
- Command translation (HTTP → sacli)
- Execution via child_process with error handling
- Response formatting and error translation
- Comprehensive logging and audit trail

// Security Features:
- No Docker socket exposure to backend
...
**2. Backend-Proxy Communication:**
```javascript
// axios-based HTTP client:
- RESTful API calls to VPN Profile Proxy
...
```javascript
// dockerode library usage (for container management only):
- Container lifecycle management
- Container inspection
- Log retrieval  
- Health monitoring
- Network configuration
// Note: Not used for OpenVPN operations
```
...
| 5 | Proxy Protocol Security | HIGH | Implemented authentication, rate limiting, input validation |
...
│   ├── Port: 3000
│   ├── Volume: logs
│   ├── Depends on: mysql, vpn-profile-proxy
...
│
├── vpn-profile-proxy (Node.js)
│   ├── Port: 8088 (internal only)
│   ├── Volume: proxy_logs
│   ├── Depends on: openvpn-server
│   ├── Environment: API_KEY, OPENVPN_CONTAINER
│   └── Healthcheck: /api/status endpoint
│
└── openvpn-server (OpenVPN AS)
    ├── Ports: 943, 444, 9443, 1194
...
- **Problem**: Need secure communication between backend and OpenVPN AS without Docker socket exposure
- **Solution**: Implemented VPN Profile Proxy service with authentication, rate limiting, and secure HTTP communication
- **Outcome**: Secure integration with 99%+ success rate, eliminated Docker socket security risks
...
**Challenge 5: VPN Profile Proxy Implementation**
- **Problem**: Secure communication between backend and OpenVPN AS without privileged Docker socket access
- **Solution**: Custom VPN Profile Proxy service with authentication, input validation, and rate limiting; RESTful API endpoints
- **Outcome**: Secure, scalable communication eliminating Docker socket vulnerabilities
...
**Challenge 2: Email Verification Flow**
